...
The full candidate suite passed 4,005 tests across 153 files. Run `npm test -- --run`. Tests that inspect backend prompt files require the sibling `tabmail-backend` checkout. Install the locked development dependencies with `npm ci --ignore-scripts`.
