...
- **[Full ADR](Companion/Decisions/Active/adr-022-startup-uid-fts-membership-fingerprints.md)** — bounded lazy pages feed UTF-8-byte/SQLite-`BINARY` digests; unchanged verified IMAP folders first use UID-only membership plus a fresh native proof, while changed/unknown/non-IMAP folders take the full exact projection. Memo v3 distinguishes before-first from valid key 0 and binds cursors/backoff to exact local+native proof and earned epochs. A writer-preferred coordinator covers manual cleanup and coalesces exclusive mutation attempts into one post-release invalidation of ephemeral reconciliation evidence; a generation-owned private retry persists its marker before waking normal work, cancels on runtime disable, and re-arms retained intent on same-generation re-enable. Data-sized fingerprints/storage stay outside short fences, and terminal-page/digest boundaries preempt. Fresh canonical inventory prunes removed identities; ⛔ 2026-09-11: empty/partial inventories remove NOTHING for accounts absent from them (cold-start wipe, hotfix 1.7.7); legacy colon-overlap ambiguity fails closed for helpers without ADR-024 capability. One phase-tagged active-folder proof prevents repeated large-folder scans without arbitrary cache cutoffs; drain rejection/abandonment releases it across bounded folder fairness while retaining durable queue work. Strict storage serialization, race-safe marker clear, ≥50%-idle scheduling, aggregate-only stored telemetry, and a shared `IMAPDeleted|Expunged` proof predicate preserve huge-profile noninterference; bodies remain queue→drain→`SafeGetFull`, native helper ≥0.11.0. Exact-mode terminal verification can reuse the initial native digest only under an unchanged membership epoch and the one-page slice budget.

## ADR-023: Native FTS Signing-Key Cutover Uses One Bridge Release, Then Revokes the Previous Key

- **[Full ADR](Companion/Decisions/Active/adr-023-native-fts-routine-signing-key-cutover.md)** — v0.11.1 is the old-signed bridge that trusts both public keys; after the bridge and reinstall UI were verified, the pending signer was promoted on 2026-08-13. v0.11.2 removes the previous public key and trusts only the promoted signer. Thunderbird 1.7.2+ marks pre-0.11.1 helpers `unsupported`, stops FTS, and offers a live-detected reinstall that preserves the existing index.

## ADR-024: Opaque Native Folder Membership Resolves Legacy Composite-Key Ambiguity

- **[Full ADR](Companion/Decisions/Active/adr-024-opaque-native-folder-membership.md)** — generation-bound Phase-A `hello`/update/`init` and post-init operational gates make capable writes connection-atomic; bootstrap ownership is revalidated across delayed update fetches so a stale generation cannot init, disconnect, or poison its replacement. Successful `updateRequest` is the intentional-exit commit point: the old host never initializes or publishes availability, and its disconnect reconnects the new generation regardless of banner timing; failure/no-update remains generation-checked. Owned async failure closes only its captured port, while coalesced exact-promise cleanup keeps synchronous connection failure retryable without clearing a newer generation. Native `folderId` carries an app-owned `tm-folder:v1:` exact escaped `[accountId,folderPath]` identity beside unchanged raw `msgId`, never a Thunderbird session ID. Bounded metadata slices plus a stable global state pass earn session-local exact equality, one native page per scheduler slice with O(1) volatile hash/cursor state. Stale owners are fenced orphans ONLY when their account is present in the fresh inventory — `accounts.list(true)` returns what is LOADED, and a cold partial inventory wiped ~58k rows on 2026-09-10 (hotfix 1.7.7, `unloadedAccountRowsKept`); conflicts/null/unresolved/storage uncertainty fail closed. Batched actual-live folder evidence preserves IPv6/Unicode/NFC-NFD tails, drains/deduplicates pagination, applies all rows in one matched folder, and rejects two-folder ambiguity. Old helpers retain ADR-022 fallback; no native session cache, body fetch, global reindex, v2 codec, or total-work cap. A stale removal invalidates the page digest and may restart earlier native pages on the next slice.
