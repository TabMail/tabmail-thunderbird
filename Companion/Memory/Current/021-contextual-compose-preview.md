...
Owner-authorized #45 recovery: empty/whitespace parsed edit bodies retry once with an explicit Body label requirement and tools disabled. A second empty result returns an error without proposed metadata/history. Cmd-K keeps the instruction and shows a retryable error; dismissed body results cannot apply, and conversation history commits only after a successful native body transaction. No heuristic unlabeled-body extraction was added.

Cmd-K recovery alerts share the existing action shadow root so visible error text cannot enter serialized mail. Tests cover failed recovery and a successful retry through the actual background handler. The inherited eager recipient-delta writer can still mutate recipients after dismissal/body refusal; #49 tracks that separate operation-lifecycle issue. Recipient functionality remains unchanged in this GUI work.

The disabled Enable control also keeps its text in a shadow root: being outside BODY alone does not exclude it from Thunderbird draft encoding. A refused native Cmd-K edit keeps the existing focus handoff and then reopens the ordinary editor with the instruction and a shadow-root error, provided no newer body edit or IME composition superseded it. Successful application keeps the previously verified caret sequence unchanged.
