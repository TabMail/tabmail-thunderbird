...
> **Correction (2026-09-12, verified against `agent/modules/contextMenus.js` at 53ed2c8):** the raw `idb.remove` bypass in `clearActionCache` is reached from the **"Recompute Action"** context-menu item (`recomputeAction` → `clearActionCache`), not from "Remove TabMail tags", which calls `applyPriorityTag(…, "none")` and is fully routed through `actionCache`. The stale-chip symptom therefore appears during the window between a recompute's clear and its new classification landing. This bypass is now closed: action mutations and clears commit through `actionCache`, project native state, and refresh both chip surfaces before scheduling delayed sorting.
