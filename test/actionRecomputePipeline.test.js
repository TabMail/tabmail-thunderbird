...
vi.mock('../agent/modules/summaryGenerator.js',()=>({getSummary:async()=>({id:key,blurb:'Synthetic summary',todos:''}),purgeExpiredSummaryEntries:async()=>{}}));
...
it('recompute merged after a failed reply commits a fresh action',async()=>{
 ...
 dependencies.reply.mockRejectedValueOnce(new Error('synthetic transient reply failure')).mockResolvedValue({});
 ...
 expect(h.store['action:'+key]).toBeUndefined();expect([...h.native.values()]).toEqual(['','']);
 ...
 expect(dependencies.reply).toHaveBeenCalledTimes(2);
