...
The verified cleanup sequence in `inlineEditor.js` uses the existing instruction iframe, before removing it: keep its container transparent, make its input focusable again, focus the instruction input, restore the saved `designMode`, focus the compose content window, focus the body, then remove the popup and restore the body's saved caret color through normal cleanup. Restoring editing mode AFTER focusing the body is insufficient: the initial caret may paint while a subsequent mouse click into an HTML paragraph makes it disappear. The body caret remains transparent during the handoff to avoid dual carets. The native HTML transaction and its selection placement remain unchanged. Do not replace this with only a body `focus()` after removing the frame, create a second focus iframe, switch applications, or add a delayed focus-stealing timer. The apply handoff is guarded by document focus and runs synchronously while the existing execution guard suppresses popup focus-out cleanup.

Regression procedure: first generate a complete HTML draft from an empty paragraph with a separate `.moz-signature` using Cmd-K. Click into several generated paragraphs without any keyboard event or application switch, and inspect the caret across a full blink cycle. Click in blank horizontal space on the same line to keep the pointer clear of the caret; do not infer invisibility from one screenshot. Then, in a disposable HTML draft containing `Please confirm Friday.`, put the caret in the body, open Cmd-K, request a different weekday, and apply with physical Enter. Without clicking the body or switching windows, type a character and inspect the visible caret. Repeat using the clickable Edit draft action. Check that only the instruction caret is shown while Cmd-K is open, Escape returns to the body, and native Undo first removes the typed character and then restores the pre-edit draft. Repeat applications and mouse-only caret placement in the same window, including after Undo. A single flat text node is not sufficient coverage for the paragraph regression. DOM tests assert editing-mode restoration before focus, the focus-before-removal sequence, and a single existing iframe, but do not substitute for this native painter check.
...
The paragraph regression was reproduced with the exact committed pre-fix module in a disposable Beta 156.0 HTML draft. The reordered mode-before-focus module passed initial generation, repeated edits, mouse-only placement on separate paragraph lines, typing and native Undo in a clean draft. A separate plaintext draft passed apply, mouse placement, and Escape dismissal. An already-affected window did not recover merely from loading the module and applying another edit: verify prevention in a fresh compose window, and do not claim live module replacement repairs pre-existing native editor state. The regression assertions fail against the previous module (`window:off`) and pass with restoration before focus (`window:on`).
