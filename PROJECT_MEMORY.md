...
- **[Detail](Companion/Memory/Current/033-cold-start-inventory-absence-is-not-deletion-evidence.md)** — `browser.accounts.list(true)` returns what Thunderbird has LOADED, not what exists; ADR-024's membership state pass + orphan sweep removed ~58k native rows (whole archive trees) because owner `folderId`s were absent from a partial inventory. Both removal sites now require the row's `accountId` to be in the inventory (`_folderReconTrustedAccountIds`, telemetry `unloadedAccountRowsKept`); an empty inventory removes nothing. The legacy path's `ensureAccountChecked` guard was dropped in the port (MIS-018). Recovery: Full Maintenance Scan / reindex. 1.7.8: the LEGACY (non-`folderMembershipV1`) sweep branch had the same hole via the global recheck; guard now applies to both branches, and the `gone:`/`ghostAcct:` blessing fixtures were re-homed to a loaded account.

### 2026-09-14 — All-day calendar items are DATEs with no zone: key by `startDay`, never the epoch; the bridge has ONE window filter (PR #48, #47)
- **[Detail](Companion/Memory/Current/034-all-day-calendar-date-has-no-zone-frame.md)** — ical.js skips zone conversion for `isDate`, so `toEpochMsUTC`/`nativeTime` of an all-day start is UTC midnight and every local rendering names the PREVIOUS day west of UTC. Bridge emits `isAllDay`/`startDay`/`endDay` (0-based `month`!) and an all-day `RECURRENCE-ID` as `${day}T00:00:00`; `calendar_search`/`calendar_read`/`calendar_event_read` key and render all-day rows from the day digits. `queryCalendarItemsInternal` no longer re-filters the provider's `[start, end)` answer with `calIDateTime.compare` — `compare` treats a DATE as equal to any same-day date-time, so the ±60 s `start_iso` lookup window was dropped. Pinned by `test/calendarAllDayFrame.test.js` (TZ=America/Vancouver, consumers + the real bridge in `node:vm` via `test/helpers/calendarBridgeHarness.js`).
...
### 2026-09-01 — Thunderbird de-XPCOM'd calendar: `cal.manager`/`cal.timezoneService`/`cal.icsService`, never `Cc[]`
- **[Detail](Companion/Memory/Current/030-calendar-de-xpcom-cal-namespace.md)** — every calendar feature died on TB 154: `@mozilla.org/calendar/manager;1` + `timezone-service;1` now resolve to module-level **singleton instances** that XPCOM still calls `new` on (`TypeError: … is not a constructor` → `NS_ERROR_XPC_GS_RETURNED_FAILURE`), and `ics-service;1` was unregistered outright (`itip-service;1` too; bare `itip-transport;1` never existed — only `?type=email`). No in-tree caller resolves **those three** through `Cc[]` any more, but that is NOT a blanket rule — TB 154 still has ~38 `Cc[]` calendar lookups (ics-parser, alarm-service, deleted-items-manager…), so never "sweep away the rest". `cal.manager`/`cal.timezoneService`/`cal.icsService` resolve on both the old lazy-service-getter and current ESM layouts, covering 140.0–155.*. Construction contracts (event/attendee/recurrence-*/datetime/ics-serializer/itip-item) still work and stay on `createInstance`. `getCalNamespace()` has **no** module-level cache on purpose — `importESModule` already caches, so a cache is pure added state in the ONE shared experiment global. Pinned by `test/calendarServiceAcquisition.test.js` (18 tests, `node:vm` execution + `acorn` AST census). **3 review rounds of test findings, all one shape: a subset map, a shape guard and a call-existence guard each read like a total guard in a passing run.** Assert the whole enumeration and assert BEHAVIOUR — a pinned call site proves the call, not that its result is used. 2 computed-form mutants survive by decision; execute a consumer, never add another regex.

### 2026-08-22 — Opaque native folder membership resolves colon-overlap keys (ADR-024)
- **[Detail](Companion/Decisions/Active/adr-024-opaque-native-folder-membership.md)** — unchanged raw `msgId` gains a relation whose `folderId` is the app-owned, versioned, exact escaped `[accountId,folderPath]` tuple; session `MailFolder.id` is transient only. Each native generation has separate Phase-A hello/update/init and successful-init operational gates, with coalesced initialization, exact-promise failure cleanup (including synchronous connection throws), captured-port disconnect on owned async failure, ownership checks across delayed update fetches, and general RPCs blocked until operational; stale bootstrap work cannot init, disconnect, or poison its replacement. Successful update response is the intentional-exit commit point, so only the replacement initializes/publishes availability across any banner timing, while failure/no-update stays generation-checked. Bounded metadata/global-state slices gate session-local cutover, one native page per scheduler turn with O(1) volatile streamed-digest state. Stale owners are fenced orphans; conflicts/null/unresolved rows fail closed. Batched actual-live folder evidence preserves Unicode/IPv6/NFC-NFD tails, filters accounts, drains/deduplicates pagination, applies every row in one matched folder, and rejects two-folder ambiguity. No native session cache, unbounded fingerprint, body reindex, codec, or total-work cap.
...
- **[Welcome background wake](Companion/Memory/Current/welcome-background-wake.md)** — Incomplete onboarding window reuse requires `windows.getAll({ populate: true })`; otherwise every fresh background can miss the retained wizard.
