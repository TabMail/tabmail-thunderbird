...
- **Install-prompt feature (shown only when `available === false`; `null` never nags):** toolbar **badge** (`agent/modules/icon.js setFtsHelperBadge`, set in `chat/background.js` after `initFtsEngine()`), **popup banner** `#fts-missing-warning` (`popup/popup.js updateFtsHelperPrompt()`), **settings CTA** `#fts-not-installed` (`config/modules/fts.js updateFtsStatus()`). All open `https://tabmail.ai/download#fts-helper` via `browser.windows.openDefaultBrowser`.
